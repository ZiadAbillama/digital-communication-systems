...
t  = linspace(-T_fixed/2, T_fixed/2, 2000);      % one full period window (analysis domain)
xt = sign(sin(2*pi*2*t));                        % signal on that window

for i = 1:numel(n_values)
    n = n_values(i);
    [xhat, ~] = ffs(xt, t, n, T_fixed);          % integrates over [-T/2, T/2] internally
    E_n(i) = trapz(t, abs(xt - xhat).^2);        
end

%Case 2: Fixed n, vary T
n_fixed = 50;
T_values = [0.6 0.9 1.3 2.5 3.7];
